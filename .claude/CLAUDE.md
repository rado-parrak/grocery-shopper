...
- Order submission IS exposed via this MCP (confirmed by the live round-trip, 2026-07-31) — it is not platform-excluded, so every skill's own instructions must explicitly refuse to call any checkout/order/payment/claim tool (see `project-knowledge/mcp-degradation.md` "Forbidden tools — never call"); this is a policy-level defense, not a structural guarantee, and every *other* mutating call still needs the discipline above precisely because nothing about the server's reliability is guaranteed.
